...
cutadapt <- "/home/louis/anaconda3/envs/dada2_pip/bin/cutadapt" # Path to the executable
...
for (i in 1:length(colnames(asv_tab))) {
  if (str_detect(colnames(asv_tab)[i], "BLNX", negate = FALSE) == "TRUE") {
    vector_for_decontam[i] = str_detect(colnames(asv_tab)[i], "BLNX", negate = FALSE)
  } else {
    vector_for_decontam[i] = str_detect(colnames(asv_tab)[i], "_B_", negate = FALSE)
  }
}
...
tax[row.names(tax) %in% contam_asvs, ]
...
write(asv_fasta_no_contam, file.path(path_results, "ASVs-no-contam.fa"))
write.csv(asv_tab_no_contam, file.path(path_results, "ASVs_counts-no-contam.csv"), row.names = FALSE)
write.csv(asv_tax_no_contam, file.path(path_results, "ASVs_taxonomy-no-contam.csv"), row.names = FALSE)
